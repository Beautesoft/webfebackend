...
TreatmentAccount,DepositAccount,PrepaidAccount)
...
def receipt_calculation(request, daud):
    ...
    print(request.user,"2 request.user")
...
def receipt_calculation(daud):
    ...
    print(request.user,"3 request.user")
    fmspw = Fmspw.objects.filter(user=request.user,pw_isactive=True).first()
    site = fmspw.loginsite 
    calcgst = 0
    if gst:
...
def GeneratePDF(self,request, sa_transacno):
        ...
        sub_data = {'total_qty':str(tot_qty),'trans_amt':str("{:.2f}".format((tot_trans))),
        'deposit_amt':str("{:.2f}".format((tot_depo))),'total_balance':str("{:.2f}".format((tot_bal))),
        'subtotal':str("{:.2f}".format((tot_trans))),'billing_amount':"{:.2f}".format(float(billable_amount)),
