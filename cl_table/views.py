    ...
    def create(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                k = serializer.save(site_code=site.itemsite_code,cust_code=cus_code,cust_sexes=gender.itm_code if gender else None, cust_joindate=timezone.now())
                if k.pk:
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            if master_ids and appointment:
                ...
                appt_data = {'appt_date':appt_date,'cust_name':data['cust_name'],'cust_id':data['cust_noid'],'booking_status':data['appt_status'],
                ...
                'item_name':master_ids.course,'Item_Codeid': master_ids.Item_Codeid.pk,'add_duration':add_duration,
    ...
    def list(self, request):
        try:
            ...
            if par_lst:
                ...
                if querylst !=[]:
                    queryset = Treatment.objects.filter(pk__in=querylst,cust_code=cust_obj.cust_code,Site_Codeid__pk=site.pk).order_by('-pk')
                    total = len(queryset)
                    state = status.HTTP_200_OK
                    message = "Listed Succesfully"
                    error = False
    ...
    def list(self, request): 
        ...
        # if self:
            ...
            if emp_queryset:
                ...
                error = False
                data = None
                result=response(self,request, emp_queryset,total,  state, message, error, serializer_class, data, action=self.action)
                # print(result,"result")
                v = result.get('data')
                # print(v,"v")
                d = v.get('dataList')
                        ...
                        for data in serializer.data:
                            ...
                            if master_ids:
                                ...
                                if appt_ids.new_remark:
                                    remark = True
                                    remark_val = "["+str(data['new_remark'])+" - "+"Remark By: "+str(empobj.display_name)+" - "+str(apptdate)+"]"
                                ...
                                if data['appt_status'] in primary_lst:
                                    ...
                                    'endDate':endDate,'cust_name':data['cust_name'],'cust_phone':data['Cust_phone'],
                                    'cust_refer':data['cust_refer'],'status': data['appt_status'],'color': statusval['color'],
